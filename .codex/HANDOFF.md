...
1. Переработать Roll-препятствие в `Level_01`.
   GUI-проверка отклонила фикс из `8585e19`: персонаж может оказаться внутри
   obstacle, а увеличение passage speed проблему не решает. Если collider
   перекрывает весь проход, прежнее выталкивание было нестабильным tunneling,
   а не физическим прохождением.
   Подтверждено: collider расположен только сверху, физический проход под ним
   существует. Согласован отдельный `PlayerLowPassage`: после завершения Roll
   он сохраняет малую капсулу, проигрывает замедленный `Roll_crouch_loop` и
   позволяет вручную выйти в любую сторону. Стоячая форма возвращается только
   после clearance-проверки; Roll-инвульнерабельность в этом состоянии не
   сохраняется.
