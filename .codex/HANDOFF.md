...
1. Выполнить GUI smoke-test нового Roll low-passage flow.
   После завершения обычного Roll под низким потолком `PlayerLowPassage`
   сохраняет малую капсулу, проигрывает `Roll_crouch_loop` со скоростью `0.55`
   и позволяет вручную двигаться во все стороны со скоростью `1.5`.
   Физическое зажатие устранено пользователем: нижняя граница collider
   `Obstacle_roll` поднята на `0.2 м`, дав запас над Roll-капсулой.
   Стоячая форма теперь возвращается после `0.1 с` стабильного clearance,
   только в фазовом окне `0.12` около начала/конца `Roll_crouch_loop` и после
   повторной clearance-проверки.
   Roll-инвульнерабельность, прыжок, атака и новый Roll под потолком отключены.
   Forced passage speed и автоматическое выталкивание удалены.
   Проверить вход с обеих сторон, остановку под потолком, разворот назад,
   полный выход, отсутствие прокрутки при вставании и временно удлинённый
   obstacle.
   Design: `docs/superpowers/specs/2026-07-29-roll-low-passage-design.md`.
   Plan: `docs/superpowers/plans/2026-07-29-roll-low-passage.md`.
   Focused и Godot 4.7 compatibility contracts проходят.
