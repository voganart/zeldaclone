...
# ... (Остальные функции смены сцен оставляем без изменений) ...
...
# --- НОВЫЕ МЕТОДЫ ДЛЯ ГЕЙМПЛЕЯ ---

## Просто затемнить экран (для респавна)
func fade_screen_to_black(duration: float = 0.5):
	var tween = create_tween()
	tween.tween_property(color_rect, "color:a", 1.0, duration)
	await tween.finished

## Убрать затемнение
func fade_screen_from_black(duration: float = 0.5):
	var tween = create_tween()
